---
title: "Provincial Groundwater Observation Well Network (PGOWN) February 2020"
...
```{r provincial plots, include = TRUE, echo= FALSE, fig.align="center", fig.height=8}
#Add provincial snap shot figures as per Jillians word doc
p1 <- well.stats %>% group_by(report_data) %>% summarize(no.active = sum(no.active.wells)) %>%
  ggplot(aes(report_data, no.active)) +
...
y.lim.max <- 48
p2 <- well.stats %>% mutate(mth.ave = if_else(mth.ave > y.lim.max, y.lim.max, mth.ave)) %>%
ggplot(aes(report_data, mth.ave, fill = Region)) +
...
p3 <- ggplot(well.stats, aes(report_data, 100 - pc.gth.7, fill = Region)) +
  geom_col(position = "dodge") +
  ylab("%") +
  xlab("") +
...
grid.arrange(p1, p2, p3, nrow = 3, heights = unit(c(6.5, 6.5, 6.5), units = c("cm", "cm", "cm")))

```


The target of publishing publicly available water level data that has been validated within the past 7 months was implemented in February 2015. The South Coast Region and the South Area implemented a full-time, dedicated Groundwater Technician in January 2016, indicated via a green dot.


## Regional Snapshots

```{r plots, include = TRUE, echo= FALSE, fig.align="center", fig.width=7, fig.height=3}
#define the location of tick marks along the date axis (Feb/Jul)
date.ticks <- unique(well.stats$report_data)
date.ticks <- sort(date.ticks)

## Plot 2 : create overall summary with all data and years.
ggplot(well.table, aes(report_data, 100 - pc.gth.7)) +
  geom_bar(stat = "identity") +
  ylim(0,100) +
