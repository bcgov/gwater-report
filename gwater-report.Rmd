...
grad.yrs = c("2020-02-01","2020-07-01","2019-02-01","2019-07-01")

library(lubridate)

graded_data <- well.stats %>%
  mutate(report_yr = year(report_date)) %>%
  filter(report_yr > 2018)

ggplot(graded_data, aes(report_date, pc.grad, color = Region, fill = Region, shape = Region)) +
  facet_wrap(~Region)+
  geom_line() +
  ylab("% Graded") +
  xlab("") +
...
*This assessment was completed July 10th, 2020 and is current as of this date.
